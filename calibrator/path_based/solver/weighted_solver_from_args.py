#!/usr/bin/env python3
"""
Weighted Least-Squares 3-DOF Extrinsic Calibration with Command-Line Covariance

This script performs weighted least-squares optimization using fixed covariance
...
def load_odometry_file(filename):
    """Load odometry data from file (4-column format: timestamp x y theta)."""
    try:
        data = np.loadtxt(filename, skiprows=1)  # Skip header
        if data.shape[1] != 4:
            print(f"Error: Expected 4 columns in {filename}, got {data.shape[1]}")
            sys.exit(1)
...
def residual_function(x_guess, wheel_relative_transforms, lidar_relative_transforms, fixed_covariance):
    """
    Weighted residual function for the least squares optimizer.
    
    This function implements the kinematic constraint with weighting:
    Error_matrix = inv(T_wheel) * X_guess * T_lidar * inv(X_guess)
    
    The residuals are weighted using the inverse of the fixed covariance matrix.
    ...
    Args:
        ...
        fixed_covariance: Fixed 3x3 covariance matrix for weighting
    
    Returns:
        Flattened array of all weighted residual errors
    """
    ...
    # Calculate weight matrix (inverse of covariance)
    # Use pseudo-inverse for numerical stability
    W = np.linalg.pinv(fixed_covariance)
    
    # Compute Cholesky decomposition of W for efficient weighting
    # W = L * L^T, so weighted_residual = L^T * residual
    try:
        L = np.linalg.cholesky(W)
        L_T = L.T
    except np.linalg.LinAlgError:
        # If Cholesky fails, fall back to matrix square root
        eigenvalues, eigenvectors = np.linalg.eigh(W)
        eigenvalues[eigenvalues < 0] = 0  # Handle numerical errors
        L_T = eigenvectors @ np.diag(np.sqrt(eigenvalues))
    
    # List to store all weighted error vectors
    all_weighted_errors = []
    ...
    for T_wheel, T_lidar in zip(wheel_relative_transforms, lidar_relative_transforms):
        ...
        # Apply weighting: weighted_error = L^T * error
        weighted_error = L_T @ error_vector
        
        # Append weighted error vector
        all_weighted_errors.extend(weighted_error)
    
    # Return flattened array of all weighted errors
    return np.array(all_weighted_errors)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Weighted Least-Squares 3-DOF Extrinsic Calibration with Command-Line Covariance'
    )
    parser.add_argument('--var_x', type=float, default=4e-04,
                        help='Variance for x-axis (default: 0.000025)')
    parser.add_argument('--var_y', type=float, default=4e-04,
                        help='Variance for y-axis (default: 0.000025)')
    parser.add_argument('--var_theta', type=float, default=4e-04,
                        help='Variance for theta-axis (default: 0.000004)')
    
    args = parser.parse_args()
    
    # Construct fixed covariance matrix from command-line arguments
    fixed_covariance = np.diag([args.var_x, args.var_y, args.var_theta])
    
    print("=== Weighted Least-Squares 3-DOF Extrinsic Calibration ===")
    print("Using fixed covariance from command-line arguments\n")
    print(f"Covariance diagonal elements:")
    print(f"  var_x:     {args.var_x:.6e}")
    print(f"  var_y:     {args.var_y:.6e}")
    print(f"  var_theta: {args.var_theta:.6e}")
    print(f"\nCovariance matrix:")
    print(fixed_covariance)
    
    # Load odometry data (4-column format)
    ...
    # Run the optimizer with weighted least squares
    print("\nRunning weighted non-linear least squares optimization...")
    ...
    result = least_squares(
        residual_function,
        initial_guess,
        args=(wheel_relative_transforms, lidar_relative_transforms, fixed_covariance),
    ...
    print(f"  Final cost (sum of squared weighted residuals): {result.cost:.8f}")
    print(f"  Number of function evaluations: {result.nfev}")
    print(f"  Number of residuals: {len(result.fun)}")
    print(f"  RMS weighted error: {np.sqrt(result.cost / (len(result.fun) / 3)):.6f}")
    print("="*60)
    ...
    plt.title(f'Weighted Least-Squares Calibration (Command-Line Covariance)\n' +
    ...
    # Save and show plot
    plt.savefig('calibration_weighted_result.png', dpi=200)
    print("Verification plot saved as 'calibration_weighted_result.png'")
    plt.show()
